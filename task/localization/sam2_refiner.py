...
class Sam2Refiner(BaseTask):
    ...
    def _init_transformers_backend(self, segmenter_model, devices):
        ...
        print(
            ...
            f"replicas={total_replicas} batch_size={self.args.get('batch_size', 4)} "
    ...
    def _process_batch(self, batch_items):
        ...
        boxes_per_image = [self._masks_to_bboxes(x[3]) for x in valid_items]
        per_image = self._infer_transformers_batch([x[2] for x in valid_items], boxes_per_image)
